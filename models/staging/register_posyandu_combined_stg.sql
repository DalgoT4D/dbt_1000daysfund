...
--   source             : 'register_posyandu' | 'epus' | 'pemerintah'
-- Sources are disjoint (no shared visits). year/quarter are DERIVED here from
-- kunjungan_tanggal in every branch, so the combine does not depend on any
-- upstream carrying those columns. Pemerintah keeps its duplicate rows in its own
-- table; they are filtered OUT here via `where not is_duplicate`. Each branch
-- selects columns BY NAME, fills its own program's columns and typed-NULLs the
-- other's; column order is identical across all branches. A final wrapper derives
-- kasus_check / kasus from the four condition flags.
...
unioned as (
    ...
    union all
    select * from pem_bumil
    union all
    select * from pem_baduta
...
select
    *,
    ...
    -- All four flags are TRUE = problem; ibu_kek_check is TRUE = KEK case in every source.
    nullif(
        concat_ws(
            ' + ',
